...
typedef enum find_status_t {fs_ok, fs_next, fs_find, fs_skip, fs_err} find_status;
		...
		size_t stringLen)
{
	...
	if (fs_ok != ctx->find_status)
	{
		...
		if (fs_find == ctx->find_status
				&& strncasecmp(ctx->path[ctx->current_depth], (const char *)stringVal,
...
stream_get_key_handl *init_json_stream_get_key(const char **path)
{
	...
	ctx->return_str = NULL;
	s_handler->ctx = ctx;
	s_handler->yajl_handle = yajl_alloc(&callbacks, NULL, (void *) s_handler->ctx);
	yajl_config(s_handler->yajl_handle, yajl_allow_comments, 1);
	return s_handler;
}

static char *json_get_stream_key(stream_get_key_handl *s_handler,
